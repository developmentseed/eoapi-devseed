"""Optional Prometheus metrics for the raster service."""
...
_ROUTE_RULES: list[tuple[re.Pattern[str], str]] = [
    ...
    (re.compile(r"^/searches/builder$"), "builder"),
...
def resolve_operation(method: str, route: str | None) -> str:
    """Map a request to a low-cardinality raster operation label."""
...
REQUESTS = _metric_or_none(
    lambda: Counter(
        "http_requests_total",
        "Total HTTP requests by raster operation.",
...
LATENCY = _metric_or_none(
    lambda: Histogram(
        ...
        "HTTP request latency by raster operation.",
